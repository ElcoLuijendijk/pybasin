...
def addErodedAltenaFms(well, erodedATTable, memberTable, 
    ...
    for i, s in enumerate(normalSuccession):
        if s in strat_units:
            top_preserved_AT = s
    ...
    # base ages of AT formation members
    
    ATage_base = np.array([199.6, 183., 177.6, 170.6, 170.1,
                            168.2, 165.4, 164.7, 164., 162.8, 161.2])
    # top ages of AT formation members
    ATage_top = np.array([  183., 177.6, 170.6, 170.1, 168.2, 165.4,
                            164.7, 164., 162.8, 161.2, 159.15]) 
    ATaddIndex = strat_units.index(top_preserved_AT) + 1

    # set number of eroded formations
    N_formations_to_add = ( len(normalSuccession) -
                            normalSuccession.index(top_preserved_AT)-1)
                            
    print '\tadding %s strat units.' %N_formations_to_add
    # check if top unit much lower thickness than average
    diff_topATunit = ATcellSize[ - N_formations_to_add - 1]\
                        - cellSize_initial_all[ATaddIndex - 1]
    
    for i,s in enumerate(strat_units):
        print '%s, %0.0f' %(s,cellSize_initial_all[i])
    ...
    lateCret_erosion_base = np.arange(lateCret_erosion_start,
                                        lateCret_erosion_end,
                                        (lateCret_erosion_end -
                                        lateCret_erosion_start)/
                                        (N_formations_to_add + 1))
    lateCret_erosion_top = np.append(lateCret_erosion_base[1:], lateCret_erosion_end)
    
    # find which formations to add, start with oldest
    added_thickness = np.zeros((len(normalSuccession)))
    ...
    for j in xrange(N_formations_to_add):
        ...
        if totalATerosion >=  (maxErosion - 1):
            print '\tmax erosion reached. not adding %s'\
                    %normalSuccession[stratIndex]

        else:
            print '\tadding %s'\
                    %normalSuccession[stratIndex]
            added_thickness[j] = ATcellSize[stratIndex]
            totalATerosion += added_thickness[j]
            
            # check if erosion exceeds max erosion value
            if totalATerosion >= (maxErosion - 1):
                diff = totalATerosion  - maxErosion[0]+1
                added_thickness[j] = added_thickness[j] - diff
                totalATerosion -= diff

    j = 0
    for i in xrange(N_formations_to_add):
        
        stratIndex = -(i+1)
        if added_thickness[stratIndex] > 0:
            ...
            # update stratigraphic codes table
            strat_units.insert( ATaddIndex,
                                normalSuccession[stratIndex])
            strat_units.insert( ATaddIndex + (j*2) + 1,
                                ('-' + normalSuccession[stratIndex]))
            
            # add eroded formations to initial cell size array:
            cellSize_initial_all = np.insert(   cellSize_initial_all,
                                                ATaddIndex,
                                                added_thickness)
            cellSize_initial_all = np.insert(   cellSize_initial_all,
                                                ATaddIndex + (j*2) + 1,
            ...
            cellSize_present_all = np.insert(   cellSize_present_all,
                                                ATaddIndex,
                                                added_thickness)
            cellSize_present_all = np.insert(   cellSize_present_all,
                                                ATaddIndex + (j*2) + 1,
            ...
            age_base = np.insert(   age_base, ATaddIndex,
                                    ATage_base[stratIndex])
            age_base = np.insert(   age_base, ATaddIndex + j + 1,
                                    lateCret_erosion_base[stratIndex])
            
            # add top ages of Altena formations:
            age_top = np.insert(age_top, ATaddIndex,
                                ATage_top[stratIndex])
            age_top = np.insert(age_top, ATaddIndex + j + 1,
                                lateCret_erosion_top[stratIndex])
    
            print '\tadded unit %s, th=%0.0f, sum eroded thickness = %0.0f'\
                %(normalSuccession[ stratIndex], added_thickness[stratIndex],
                    totalATerosion)
            
            j+=1
    
    for i,s in enumerate(strat_units):
        print '%s,%0.2f,%0.2f' %(s,age_base[i],age_top[i])
    
    pdb.set_trace()
    
    
    return  subsidence_code, strat_units, cellSize_initial_all,\
            cellSize_present_all, age_base, age_top,\
...
def addErodedSLDNAfm(subsidence_code, strat_units,
    ...
    totalSLerosion = 0
    SLDNAsize_new = (lateCret_erosion[0] - totalATerosion) # new cell size of SLDNA fm.
    ...
    if 'SLDNA' not in strat_units and SLDNAsize_new>2.0:
...
def calculateGOFages(AFTages_, AFTages_min_, AFTages_max_,
    ...
    for ageDataNo in xrange(len(AFTages_)):
            ...
            if AFTages_[ageDataNo] >=  minAge and AFTages_[ageDataNo] <=  maxAge:
                difference = 0 ; difference_norm = 0
            ...
            # calculate residual
                ...
                if difference<0: ageRange = AFTages_min_[ageDataNo]
                else: ageRange = AFTages_max_[ageDataNo]
...
def addPrePermErosion(subsidence_code, strat_units,
    ...
    # modifies the subsidence, stratigraphy code  lists and the cellsize and age arrays
    ######################################################################################
    
    # find highest carboniferous form
    if 'DCCU' in strat_units:
        topCarbFm = 'DCCU'
        newCarbFm = 'DCDH'
    if 'DCDH' in strat_units:
        topCarbFm = 'DCDH'
        newCarbFm = 'DCHS'
    if 'DCHL' in strat_units:
        topCarbFm = 'DCHL'
        newCarbFm = 'DCHS'
    ...
    for i in xrange(len(strat_units)):
        if strat_units[i][:2] == 'DC':
            topCarbFm = strat_units[i]
            newCarbFm = 'DCHS'
    
    prePermIndex = strat_units.index(topCarbFm) + 1
    ...
    # update stratigraphic codes table
    strat_units.insert(prePermIndex, (newCarbFm + '-2'))
    strat_units.insert(prePermIndex + 1, ('-' + newCarbFm + '-2'))
...
def runModelScenario(
    ...
    if duration.min() <=  0:
        print 'error preparing well subsidence data, <0 duration timestep'
        for i,s in enumerate(strat_units):
            print '%s,dt=%0.2f,age= %0.2f-%0.2f Ma'\
                    %(s,duration[i],age_base[i],age_top[i])
        #print duration
        pdb.set_trace()
